...
export default function App() {
  ...
  const handleDeleteProject = async (projectId) => {
    try {
      ...
      if (remaining.length === 0) {
        ...
        showToast('Project deleted. Switched to the next available project.');
      } else {
  ...
  const handleRunBatch = async () => {
    ...
    if (activePrompts.length === 0) {
      showToast('No active prompts configured. Please activate at least one prompt.');
      return;
    }
  ...
  return (
    <div className="min-h-screen flex flex-col antialiased bg-surface-900 text-slate-200">
      ...
      <Routes>
        ...
        <Route
          path="/"
          element={<LandingView onGetStarted={() => setIsModalOpen(true)} />}
        />
