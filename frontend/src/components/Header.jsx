 ...
 * Includes project switcher (with styled delete confirmation),
 * model + rounds selectors, and batch execution trigger.
 */
export default function Header({
  ...
  // { id, name } of project pending deletion, or null
  ...
  const handleConfirmDelete = () => {
    if (pendingDelete && onDeleteProject) {
  ...
  return (
    <>
      <header ref={headerRef} className="border-b border-surface-border bg-surface-850 sticky top-0 z-40 px-6 py-3 flex items-center justify-between">
        ...
        <div className="flex items-center gap-6">
          ...
          <div className="relative">
            ...
            >
              <span className={`w-2 h-2 rounded-full ${activeProject ? 'bg-emerald-400' : 'bg-slate-500'}`}></span>
            ...
            {projectOpen && (
              <div className="absolute top-full left-0 mt-1.5 w-72 bg-surface-850 border border-surface-border rounded-xl shadow-2xl z-50 p-1.5 space-y-1">
      ...
      {/* Styled delete confirmation dialog — rendered outside the sticky header */}
      <ConfirmDialog
        isOpen={!!pendingDelete}
        title={`Delete "${pendingDelete?.name}"?`}
        description="This will permanently remove the project and all its prompts, executions, citations, and tracking history. This action cannot be undone."
